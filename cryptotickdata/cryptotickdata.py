...
class CryptoTickMultiSymbolREST(CryptoTickREST):
...
class CryptoTickHourlyMixin:
    ...
    def iter_partition(self):
        ...
        for partition in period.range("hours"):
            ...
            self.timestamp_to = partition.replace(
...
class CryptoTickMultiSymbolHourlyMixin(CryptoTickHourlyMixin):
    @property
    def active_symbols(self):
        return [s for s in self.symbols if self.partition <= s["expiry"]]


class CryptoTickMultiSymbolDailyMixin(CryptoTickDailyMixin):
    @property
    def active_symbols(self):
        return [s for s in self.symbols if self.partition <= s["expiry"].date()]
...
class CryptoTickDailyHourlyMixin:
...
class CryptoTickDailyPartitionFromHourlyMixin(
    CryptoTickDailyMixin, CryptoTickDailyHourlyMixin
):
    def main(self):
        for partition in self.iter_partition():
            data = self.get_document(partition)
            if not self.is_data_OK(data):
                ...
                if has_data:
                    ...
                    data_frame = self.process_data_frame(data_frame)
    ...
    def process_data_frame(self, data_frame):
        # Maybe 0
        if len(data_frame["index"].unique()) != len(data_frame):
