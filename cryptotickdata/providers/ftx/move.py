...
from ...cryptotickdata import (
    CryptoTickMultiSymbolDailyMixin,
    CryptoTickMultiSymbolHourlyMixin,
...
class BaseFTXMOVE(FTXMixin, CryptoTickMultiSymbolREST):
    @property
    def log_prefix(self):
...
class FTXMOVEHourlyPartition(CryptoTickMultiSymbolHourlyMixin, BaseFTXMOVE):
    def get_suffix(self, sep="-"):
        symbol = BTCMOVE.replace("-", "")
        return f"{symbol}{sep}hot"


class FTXMOVEDailyPartition(CryptoTickMultiSymbolDailyMixin, BaseFTXMOVE):
    def get_suffix(self, sep="-"):
